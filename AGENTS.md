...
- MCP wiring puts the URL and bearer token **inline** in the client config (e.g. `~/.cursor/mcp.json` `headers.Authorization`), written by `install.sh` / the `/get-token` page. Tokens are org-bound `tsk_*` API keys (invite redeem or `teamshared token mint`); legacy `teamshared_*` file auth is opt-in via `TEAMSHARED_LEGACY_TOKEN_AUTH_ENABLED` (off in production). No `TEAMSHARED_URL` / `TEAMSHARED_TOKEN` env vars are required client-side; the Cursor plugin no longer ships an env-based `mcp.json` (it was a redundant broken duplicate). Agents must not run shell commands to probe `TEAMSHARED_*` env vars — use the `health` MCP tool to check connectivity.
