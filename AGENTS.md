...
- Remote install uses one `install.sh` with interactive harness selection and a bearer-token prompt written into MCP config; no per-harness `.sh` scripts and no invite-redeem flow in the installer.
