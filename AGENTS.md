...
- **Cloud Agents** read the rule from the plugin bundle at `plugins/teamshared/rules/teamshared.mdc` (canonical copy). The legacy committed `.cursor/rules/teamshared.mdc` is gone — Cursor installs the rule globally under `~/.cursor/rules/teamshared.mdc` via `install.sh`, not per-repo. Keep `plugins/teamshared/rules/teamshared.mdc` and the onboarding-page `install/assets/cursor/teamshared.mdc` (served from that same plugin copy) in sync.
