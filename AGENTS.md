...
The server fetches its shared resources (Redis client, pgvector-backed
...
and writes back through the same `ProductionServices` ingestion path. A second worker, the **curator**
...
- **VectorStore is canonical.** Production semantic/episodic reads and writes use
  [`memory/vectorstore.py`](src/teamshared/memory/vectorstore.py). The Mem0-backed
  `semantic.py` / `recall.py` path is migration-only legacy: do not add new call
  sites. If touching it before deletion, remember Mem0 is sync and wrap calls in
  `run_in_executor`.
...
- **Legacy Mem0 2.0 kwarg contract.** `Memory.search()` and `Memory.get_all()` reject
...
- **Legacy Mem0 returns cosine distance in `score`.** Its pgvector backend stores
...
- **Read paths are agent-unscoped by default.** `memory_recall` and
 ...
 pillars. The shared-brain promise lives in `MemoryFacade.recall` /
 `SecureRetrieval.search`, where `agent=`
...
- Remote install uses one `install.sh` with interactive prompts (a harness multiple-choice selector plus a bearer-token paste). Cursor writes to global `~/.cursor` (rule + MCP config); all other harnesses write project-local MCP config under the current directory. No environment variables (`TEAMSHARED_HARNESS` / `TEAMSHARED_TOKEN` / `TEAMSHARED_INVITE`); no per-harness `.sh` scripts and no invite-redeem flow in the installer.
...
- Team production host is `https://teamshared.com` (`/health`, `/mcp`, unified install at `/install.sh` and `/install`); `mcp.teamshared.com` is retired.
...
- `teamshared` Cursor plugin at `plugins/teamshared/` bundles MCP wiring, recall rule, continual-learning hooks, and install assets; install via `curl -fsSL https://teamshared.com/install.sh` (assets served from `plugins/teamshared/install/` and `rules/` at `/install/assets/`), marketplace (`/add-plugin teamshared`), or symlink to `~/.cursor/plugins/local/teamshared`.
- Legacy names `actx`, `sptx`, `actx-memory`, and `sptx-memory` are retired; use `teamshared` for package, plugin, and MCP server naming.
- `teamshared seed` loads starter **skills** (atomic building blocks); use `--procedures` for legacy playbook seeding. Semantic and episodic memory come from `memory_remember` and session distillation.
- MCP wiring puts the URL and bearer token **inline** in the client config (e.g. `~/.cursor/mcp.json` `headers.Authorization`), written by `install.sh` or minted in the console **API Keys** page. Tokens are org-bound `tsk_*` API keys (invite redeem or `teamshared token mint`). No `TEAMSHARED_URL` / `TEAMSHARED_TOKEN` env vars are required client-side; the Cursor plugin no longer ships an env-based `mcp.json` (it was a redundant broken duplicate). Agents must not run shell commands to probe `TEAMSHARED_*` env vars — use the `health` MCP tool to check connectivity.
- Cursor does not auto-fetch remote `.mdc` rules from MCP on connect; distribute agent guidance via the plugin bundle or the bundled rule on install pages.
- Agents read the rule from the plugin bundle at `plugins/teamshared/rules/teamshared.mdc` (canonical copy). The legacy committed `.cursor/rules/teamshared.mdc` is gone — Cursor installs the rule globally under `~/.cursor/rules/teamshared.mdc` via `install.sh`, not per-repo. Install assets and the memory rule both live under `plugins/teamshared/` (`install/` + `rules/`); the server serves them at `/install/assets/*` via `teamshared.clients.install_assets`.
