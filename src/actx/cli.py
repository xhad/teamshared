"""``actx`` CLI -- the operator-facing surface.

Subcommands:

- ``actx serve [--transport http|stdio]`` -- run the MCP server.
- ``actx worker`` -- run the distillation worker.
- ``actx migrate`` -- apply SQL migrations against the configured Postgres.
- ``actx token mint <agent>`` -- issue a bearer token for an agent.
- ``actx token list`` -- list issued tokens (prefixes only).
- ``actx token revoke <prefix>`` -- revoke tokens by prefix.
- ``actx config show`` -- print effective settings (secrets redacted).
"""
...
from actx.auth import TokenStore
from actx.config import get_settings
from actx.logging import configure_logging

app = typer.Typer(no_args_is_help=True, add_completion=False, help="actx memory CLI")
...
def serve(
    ...
    host: str | None = typer.Option(None, help="Override ACTX_HOST"),
    port: int | None = typer.Option(None, help="Override ACTX_PORT"),
) -> None:
    """Run the MCP server."""
    ...
    if transport == "stdio":
        from actx.server.app import _init_state, _teardown_state, build_mcp
    ...
    from actx.server.app import build_http_app
...
@app.command()
def seed(
    agent: str = typer.Option("actx", help="Agent attribution stamped on each seed procedure"),
...
def worker() -> None:
    ...
    from actx.distill.worker import main as worker_main
...
def migrate(
    ...
    with psycopg.connect(settings.pg_dsn, autocommit=True) as conn, conn.cursor() as cur:
        cur.execute(
            """
            CREATE TABLE IF NOT EXISTS actx_migrations (
                name TEXT PRIMARY KEY,
                applied_at TIMESTAMPTZ DEFAULT now()
            )
            """
        )
        cur.execute("SELECT name FROM actx_migrations")
        ...
        for path in files:
            ...
            cur.execute(sql)
            cur.execute("INSERT INTO actx_migrations (name) VALUES (%s)", (path.name,))
