...
side-effect-only at the edges; all real work happens in ``actx.memory``.

Agent identity is resolved from the per-request bearer token (see
``actx.auth``). If the caller doesn't pass ``agent`` explicitly, we fall back
...
from actx.auth import current_agent
from actx.logging import get_logger
from actx.memory.types import MemoryKind, MemoryScope, TimeRange
...
def register_tools(mcp: Any) -> None:
    ...
    async def memory_graph_relate(
        ...
        learn a structured fact like "alice -> works_on -> actx" that vector
