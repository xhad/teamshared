...
class Settings(BaseSettings):
    """All actx runtime configuration.

    Loaded from environment variables prefixed with ``ACTX_`` (and a few
    ...
    - ``port`` reads ``PORT`` as a fallback. Railway, Render, Fly, Heroku, and
      every other PaaS that injects a port use ``$PORT``; actx-native is
      ``ACTX_PORT``. Explicit ``ACTX_PORT`` always wins over ``PORT``.
    - ``pg_dsn_override`` reads ``ACTX_PG_DSN`` or ``DATABASE_URL`` and, when
    ...
    port: int = Field(
        default=8077,
        validation_alias=AliasChoices("ACTX_PORT", "PORT"),
    )
    log_level: str = "info"
    auth_disabled: bool = False
    tokens_file: Path = Field(default=Path("./.actx/tokens.json"))

    pg_host: str = "localhost"
    pg_port: int = 5432
    pg_user: str = "actx"
    pg_password: str = "actx"
    pg_db: str = "actx"
    pg_dsn_override: str | None = Field(
        default=None,
        validation_alias=AliasChoices("ACTX_PG_DSN", "DATABASE_URL"),
        description=(
            "Full Postgres DSN; when set, supersedes the five ACTX_PG_* "
    ...
    def _hydrate_pg_parts_from_dsn(self) -> "Settings":
        """When ``ACTX_PG_DSN`` / ``DATABASE_URL`` is present, back-populate the
        five-part fields so downstream consumers (notably Mem0's pgvector
        backend in :mod:`actx.memory.semantic`) see consistent values without
        forcing operators to set each part individually.

        Idempotent: if ``pg_dsn_override`` is already absent we do nothing,
        and if any individual ``ACTX_PG_*`` was *also* set explicitly the
        ...
        if parsed.scheme not in {"postgres", "postgresql"}:
            raise ValueError(
                f"ACTX_PG_DSN/DATABASE_URL must use postgres:// or "
