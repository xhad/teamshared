...
from sptx.auth import BearerAuthMiddleware, TokenStore
from sptx.config import Settings, get_settings
from sptx.logging import configure_logging, get_logger
...
from sptx.telemetry import instrument_asgi, setup_tracing

log = get_logger(__name__)


def build_mcp(settings: Settings | None = None) -> FastMCP:
    """Build a FastMCP instance with all sptx tools registered."""
...
def build_http_app(settings: Settings | None = None) -> Starlette:
    ...
    async def health_route(request: Request) -> JSONResponse:
        try:
            from sptx.server.state import get_state
    ...
    async def lifespan(app: Starlette) -> AsyncIterator[None]:
        ...
        log.info("sptx_server_started", host=settings.host, port=settings.port)
        async with mcp_app.lifespan(app):
            try:
                yield
            finally:
                log.info("sptx_server_stopping")
