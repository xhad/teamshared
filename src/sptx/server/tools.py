...
side-effect-only at the edges; all real work happens in ``sptx.memory``.

Agent identity is resolved from the per-request bearer token (see
``sptx.auth``). If the caller doesn't pass ``agent`` explicitly, we fall back
...
from sptx.auth import current_agent
from sptx.logging import get_logger
from sptx.memory.types import MemoryKind, MemoryScope, TimeRange
...
def register_tools(mcp: Any) -> None:
    ...
    async def memory_graph_relate(
        ...
        learn a structured fact like "alice -> works_on -> sptx" that vector
