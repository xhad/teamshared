...
# Add sptx as an MCP server (replace <agent-token>)
openclaw config set 'mcp_servers.sptx-memory.url' 'https://memory.tailXXXX.ts.net/mcp'
openclaw config set 'mcp_servers.sptx-memory.headers.Authorization' 'Bearer <agent-token>'
openclaw config set 'mcp_servers.sptx-memory.timeout' 30
...
Mint OpenClaw's token on the sptx host with:

```bash
sptx token mint openclaw
```
