# Connecting OpenClaw to teamshared
...
# Add teamshared as an MCP server (replace <agent-token>)
openclaw config set 'mcp_servers.teamshared-memory.url' 'https://memory.tailXXXX.ts.net/mcp'
openclaw config set 'mcp_servers.teamshared-memory.headers.Authorization' 'Bearer <agent-token>'
openclaw config set 'mcp_servers.teamshared-memory.timeout' 30
...
Mint OpenClaw's token on the teamshared host with:

```bash
teamshared token mint openclaw
```
