...
Guarded writes land **`active` immediately** (no human approval queue). Injection
screening is audit-only; hard secrets are rejected before storage.

| Write | Typical outcome |
|---|---|
| `memory_remember` | `active` |
| `memory_skill_set` / `memory_playbook_set` | `active` |
| `memory_strategic_*` | `active` |
| `work_create` | `active` |
| Connector/extraction source | `active` (connector onboarding may still need human setup) |
