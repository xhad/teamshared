...
# --- connect (token-based: telegram) ---------------------------------------


def test_connect_telegram_creates_connection() -> None:
    services = _services()
    services.connectors.create_token_connection = AsyncMock(return_value=uuid.uuid4())
    app = _app(services)
    with TestClient(app) as client:
        resp = client.post("/v1/integrations/connect", json={
...
def test_connect_rejects_oauth_kind() -> None:
    services = _services()
    app = _app(services)
    with TestClient(app) as client:
        resp = client.post("/v1/integrations/connect", json={
...
def test_connect_rejects_missing_token() -> None:
    ...
    with TestClient(app) as client:
        resp = client.post("/v1/integrations/connect", json={
            "kind": "telegram", "name": "tg", "token": "",
        })
    assert resp.status_code == 400
    assert resp.json()["error"]["code"] == "bad_request"


def test_disconnect_telegram_skips_revoke() -> None:
    # Telegram has no OAuth revoke endpoint; disconnect should still delete the row.
    services = _services()
    cid = uuid.uuid4()
    services.connectors.get_connector = AsyncMock(
        return_value={"id": str(cid), "kind": "telegram", "config": {}}
    )
    services.connectors.get_token_bundle = AsyncMock(
        return_value=TokenBundle(access_token="123456:ABC")
    )
    app = _app(services)
    with patch(
        "teamshared.connectors.oauth.revoke_token", new=AsyncMock(return_value=None),
    ) as revoke, TestClient(app) as client:
        resp = client.delete(f"/v1/integrations/{cid}")
    assert resp.status_code == 200
    assert resp.json()["deleted"] is True
    # Telegram is not an OAuth kind → revoke must NOT be called.
