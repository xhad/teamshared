...
server, distiller, curator). No Tailscale, no self-hosted TLS,
no Caddy.
...
plus `TEAMSHARED_NEO4J_USER` / `TEAMSHARED_NEO4J_PASSWORD` on the server and
distiller.
...
> and [`railway.curator.toml`](railway.curator.toml).
...
| Var | Value |
|---|---|
| `TEAMSHARED_OBJECT_STORAGE_ENDPOINT` | S3-compatible endpoint URL (e.g. the Railway bucket URL) |
| `TEAMSHARED_OBJECT_STORAGE_BUCKET` | bucket name |
